...
import site.arookieofc.processor.validation.ValidationInterceptor;
import java.io.File;
...
public class AnnotationApplicationContext implements ApplicationContext {
    ...
    private Object createBean(String beanName, BeanDefinition beanDefinition) {
        ...
        if (instance != null) {
            ...
            // 应用拦截器（在依赖注入完成后）
            instance = applyInterceptors(instance, beanDefinition);
        }
        return instance;
    }
    ...
    /**
     * 应用拦截器到bean实例
     * @param instance 原始bean实例
     * @param beanDefinition bean定义
     * @return 可能被代理的bean实例
     */
    private Object applyInterceptors(Object instance, BeanDefinition beanDefinition) {
        Object proxiedInstance = instance;
        
        // 检查是否需要事务拦截器
        if (needsTransactionInterceptor(instance.getClass())) {
            log.debug("为bean {} 应用事务拦截器", beanDefinition.getBeanName());
            proxiedInstance = TransactionInterceptor.createProxy(proxiedInstance);
        }
        
        // 检查是否需要验证拦截器
        if (needsValidationInterceptor(instance.getClass())) {
            log.debug("为bean {} 应用验证拦截器", beanDefinition.getBeanName());
            proxiedInstance = ValidationInterceptor.createProxy(proxiedInstance);
        }
     ...
     * 判断是否为Service层的bean
     * @param clazz bean的类
     * @return 是否为Service bean
     */
    private boolean isServiceBean(Class<?> clazz) {
        // 通过类名判断（以ServiceImpl结尾）
     ...
     * @return 是否需要事务拦截器
     */
    private boolean needsTransactionInterceptor(Class<?> clazz) {
        // 检查类级别的@Transactional注解
        if (clazz.isAnnotationPresent(Transactional.class)) {
            return true;
        }

        // 检查方法级别的@Transactional注解
        Method[] methods = clazz.getDeclaredMethods();
        for (Method method : methods) {
     ...
     * @return 是否需要验证拦截器
     */
    private boolean needsValidationInterceptor(Class<?> clazz) {
        Method[] methods = clazz.getDeclaredMethods();
        for (Method method : methods) {
